...
export function toAgentDomain(api: PaperclipAgent): Agent {
...
export function toAgentDomainList(apis: PaperclipAgent[]): Agent[] {
  return apis.map((api) => toAgentDomain(api));
}

export function toAgentPersistence(domain: Agent): PaperclipAgent {
  return {
    id: domain.id,
